...
vector_n <- (4:13)*2+1
vector_i <- 2^(1:20)
interseccion <- intersect(vector_n, vector_i)
interseccion
...
-   x1 es un vector de tipo "character" (caracter). Todos sus elementos están definidos como texto entre comillas ("1", "2", etc.).
-   x2 es un vector de tipo "numeric" (numérico) o "double" (doble precisión). Esto se debe a que la función log() devuelve números decimales después de calcular el logaritmo natural de los valores booleanos en el vector.
-   x3 es un vector de tipo "logical" (lógico). Esto se debe a que el operador ! aplica la negación lógica a un vector numérico, y los resultados son valores booleanos (TRUE o FALSE).
-   x4 es un vector de tipo "logical" (lógico). El código 1:3\<"a" compara cada elemento del vector 1:3 con la cadena de texto "a". Como resultado, obtendrás un vector de valores booleanos.
-   x5 es un vector de tipo "integer" (entero). El operador 1:10 genera una secuencia de enteros desde 1 hasta 10, por lo que todos los elementos del vector son números enteros.
...
```{r ejercicio7}
matrix(c("A", "B", "C", "X", "Y", "Z"), nrow = 2, ncol = 3, byrow = TRUE)
```
...
```{r ejrcicio10}
matrix(c(2 + 3i, 4 - 1i, 1 + 2i, 3 - 5i), nrow = 2)
```
...
-   ?Qué sucede si a la lista `x` le aplico `dim(x) <- c(2,3)`? obtendremos un error porque las listas en R no tienen una dimansion como lo tienen una dimension como lo tienen las matrices o arrays por lo que no se puede asignar una dimensión a la lista de la misma manera que lo harías con una matriz.

-   ?Cómo podrá llamar a ese nuevo objeto? se le podria llamar matriz X

-   ?Qué sucede si `c(list("a"), 2+3i)`? ?De qué tipo es el resultado? Cuando se ejecuta la operación c(list("a"), 2+3i), se esta combinando una lista y un vector atómico en R. El resultado será una nueva lista que contiene ambos elementos. En este caso, el resultado será una lista con dos elementos: la lista original list("a") y el número complejo 2+3i.

### Gráficos

El comando `plot` me permite graficar dos vectores de igual longtitud. El comando `par(mfrow=c(2,2))` divide el espacio de graficación en dos filas y dos columnas con lo cual yo graficar 4 gráficos sobre la misma figura. Por otro lado el comando `write(path)` permite escribir un archivo en la dirección dada por `path`, por ejemplo:
